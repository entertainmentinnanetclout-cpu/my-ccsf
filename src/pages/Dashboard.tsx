...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background" data-testid="ready-dashboard">
      <EmergencyReport />
      <motion.header initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, ease: 'easeInOut' }} className="sticky top-0 z-40 border-b border-border border-t-4 border-t-[#F2A900] bg-background shadow-soft dark:bg-primary">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              ...
              <div className="hidden min-w-0 sm:block">
                <div className="flex items-center gap-2"><Shield className="h-5 w-5 shrink-0 text-primary dark:text-white" aria-hidden="true" /><h1 className="truncate text-lg font-bold text-primary dark:text-white sm:text-xl">{BRAND.productLongName}</h1></div>
                <p className="text-xs font-medium text-muted-foreground dark:text-white/80 sm:text-sm">CCSF Student Portal</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <Button asChild size="sm" className="border border-[#F2A900]/70 bg-[#D7193F] px-2 font-extrabold text-white shadow-md hover:bg-[#B91435] sm:px-3" data-testid="open-pilot-mode"><Link to="/pilot"><FlaskConical className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Open Pilot</span></Link></Button>
              <motion.div className="hidden items-center gap-2 rounded-full border border-border bg-muted px-4 py-2 shadow-sm dark:border-white/20 dark:bg-white/10 lg:flex" whileHover={{ scale: 1.02 }}><MapPin className="h-4 w-4 text-primary dark:text-white" aria-hidden="true" /><span className="text-sm font-semibold text-primary dark:text-white">{campusLabel}</span></motion.div>
              <ThemeToggle />
              <NotificationBell />
              <Button variant={activeView === 'settings' ? 'secondary' : 'ghost'} size="icon" onClick={() => changeView('settings')} aria-label="Open account security settings"><Settings className="h-5 w-5" aria-hidden="true" /></Button>
              <motion.div whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}><Button variant="ghost" size="icon" onClick={() => void signOut()} className="hidden sm:flex" aria-label="Sign out of CCSF"><LogOut className="h-5 w-5" aria-hidden="true" /></Button></motion.div>
            </div>
          </div>
        </div>
      </motion.header>
            ...
            <div className="grid grid-cols-6 gap-1.5 sm:gap-2" role="tablist" aria-label="Student portal sections">
              {navItems.map(({ view, icon: Icon, label }) => (
                <motion.div key={view} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                  <Button role="tab" aria-selected={activeView === view} variant={activeView === view ? 'default' : 'ghost'} onClick={() => changeView(view as StudentView)} className={`w-full px-1 text-xs transition-all sm:px-3 sm:text-sm ${activeView === view ? 'bg-gradient-to-r from-primary to-secondary shadow-lg' : 'hover:bg-primary/10'}`} size="sm"><Icon className={`h-4 w-4 ${activeView === view ? '' : 'lg:mr-2'}`} aria-hidden="true" /><span className="hidden lg:inline">{label}</span></Button>
                </motion.div>
              ))}
            </div>
          </Card>
        ...
        <motion.div key={activeView} initial={{ opacity: 0, x: 20, scale: 0.97 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -20, scale: 0.97 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
          {activeView === 'home' && (
            <div className="space-y-5">
              <div className="px-4"><Card className="overflow-hidden border-[#F2A900]/55 shadow-large"><CardContent className="flex flex-col justify-between gap-5 bg-gradient-to-r from-[#002F6C] via-[#07366D] to-[#1A0D2B] p-5 text-white sm:flex-row sm:items-center sm:p-6"><div><div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.16em] text-[#F2A900]"><ShieldCheck className="h-4 w-4" />New student safety layer</div><h2 className="mt-2 text-xl font-black sm:text-2xl">In-Transit, Night Travel, Track This Phone and Campus Radar</h2><p className="mt-2 max-w-3xl text-sm text-white/70">Use opt-in live location, tappable campus profiles, the existing GPS map and one-tap institutional safety alerts from a single organised hub.</p></div><Button className="shrink-0 bg-[#F2A900] font-extrabold text-[#002F6C] hover:bg-[#F2A900]/90" onClick={() => changeView('safety')}><Radar className="mr-2 h-5 w-5" />Open Safety Hub</Button></CardContent></Card></div>
