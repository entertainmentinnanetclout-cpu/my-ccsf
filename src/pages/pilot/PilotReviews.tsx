...
export default function PilotReviews() {
  ...
  const [options, setOptions] = useState<PilotReviewOptions>({
    categories: DEFAULT_PILOT_REVIEW_CATEGORIES,
    quickCards: DEFAULT_PILOT_REVIEW_QUICK_CARDS,
  });
  ...
  const refresh = useCallback(async (showSpinner = false) => {
    ...
    try {
      const [nextReviews, nextReports, nextOptions] = await Promise.all([
        loadPilotReviews(),
        loadOwnPilotReports(),
        loadPilotReviewOptions(),
      ]);
      ...
      setForm((current) => ({
        ...current,
        category: nextOptions.categories.some((item) => item.key === current.category)
          ? current.category
          : nextOptions.categories[0]?.key ?? 'other',
      }));
    } catch (error) {
      toast({
  ...
  const resetForm = () => {
    setForm({
      ...EMPTY_FORM,
      category: options.categories[0]?.key ?? 'other',
    });
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast({ title: 'Review not saved', description: error instanceof Error ? error.message : 'Try again.', variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
            <div>
              ...
              <CardDescription className="mt-2 max-w-3xl text-white/80">
                Rate the reporting journey, select managed quick feedback and explain any issue. Reviews remain inside the controlled Pilot environment.
              </CardDescription>
            </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="shadow-large">
          <CardHeader>
            ...
            <CardDescription>Quick cards are configured by the Pilot super admin, populate the form immediately and remain editable.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-3">
              <Label>Quick feedback</Label>
              <div className="grid gap-3 sm:grid-cols-2">
                {options.quickCards.map((item) => {
                  const selected = form.quickFeedback.includes(item.label);
                  const PositiveIcon = item.sentiment === 'negative' ? AlertCircle : CheckCircle2;
                  return (
                    <button
                      key={item.id}
                      type="button"
                      aria-pressed={selected}
                      onClick={() => toggleQuickFeedback(item.label, item.category_key)}
                    ...
                    >
                      <span className="flex items-center gap-2"><PositiveIcon className="h-4 w-4" />{item.label}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select value={form.category} onValueChange={(value) => setForm((current) => ({ ...current, category: value }))}>
                  ...
                  <SelectContent>
                    {options.categories.map((category) => <SelectItem key={category.key} value={category.key}>{category.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={form.reportId} onValueChange={(value) => setForm((current) => ({ ...current, reportId: value }))}>
                  ...
                  <SelectContent>
                    ...
                    {reports.map((report) => <SelectItem key={report.id} value={report.id}>{report.reference_number} · {report.title}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea id="review-text" value={form.reviewText} onChange={(event) => setForm((current) => ({ ...current, reviewText: event.target.value }))} placeholder="Describe what worked, what failed and what should be improved." rows={6} maxLength={5000} />
            ...
            <div className="space-y-2">
              ...
              <Input id="review-screenshot" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setScreenshot(event.target.files?.[0] ?? null)} />
              <p className="text-xs text-muted-foreground">Private JPG, PNG or WebP. Maximum 5 MB.</p>
            </div>

            <div className="flex items-start gap-3 rounded-xl border border-border bg-muted/40 p-4">
              <Checkbox id="contact-permission" checked={form.contactPermission} onCheckedChange={(checked) => setForm((current) => ({ ...current, contactPermission: checked === true }))} />
              <div><Label htmlFor="contact-permission" className="cursor-pointer">Permission to contact me</Label><p className="mt-1 text-xs text-muted-foreground">Authorised Pilot staff may contact you to clarify this review.</p></div>
            </div>
        ...
        <Card className="shadow-large">
          <CardHeader><CardTitle className="flex items-center gap-2"><MessageSquareText className="h-5 w-5" />Your previous reviews</CardTitle><CardDescription>Read status changes and authorised staff responses.</CardDescription></CardHeader>
          <CardContent>
            {loading ? (
                  ...
                  return (
                    <article key={review.id} className="rounded-xl border border-border p-4 shadow-sm">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div><div className="flex items-center gap-1" aria-label={`${review.rating} out of 5 stars`}>{[1, 2, 3, 4, 5].map((star) => <Star key={star} className={`h-4 w-4 ${star <= review.rating ? 'fill-[#F2A900] text-[#F2A900]' : 'text-muted-foreground/30'}`} />)}</div><p className="mt-2 text-sm font-bold">{categoryLabels[review.category] ?? review.category}</p><p className="text-xs text-muted-foreground">{format(new Date(review.created_at), 'dd MMM yyyy, HH:mm')}</p></div>
                        <Badge variant={statusVariant(review.status)}>{PILOT_REVIEW_STATUS_LABELS[review.status]}</Badge>
                      </div>
                      {review.quick_feedback.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{review.quick_feedback.map((item) => <Badge key={item} variant="outline">{item}</Badge>)}</div>}
                      ...
                      {review.admin_response && <div className="mt-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Authorised staff response</p><p className="mt-2 whitespace-pre-wrap text-sm">{review.admin_response}</p></div>}
                      <div className="mt-4 flex flex-wrap gap-2">
                        {EDITABLE_PILOT_REVIEW_STATUSES.has(review.status) && <Button size="sm" variant="outline" onClick={() => beginEdit(review)}><Edit3 className="mr-2 h-4 w-4" />Edit</Button>}
