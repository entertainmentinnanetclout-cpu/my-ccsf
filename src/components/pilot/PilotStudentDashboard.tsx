...
export function PilotStudentDashboard({ program, participant, session }: {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      toast({
        title: 'Pilot dashboard unavailable',
  ...
  return (
    <div className="min-h-[calc(100vh-12rem)] bg-background" data-testid="ready-pilot-student-dashboard">
          ...
          <div className="grid grid-cols-5 gap-2">
            {navItems.map(({ view: itemView, icon: Icon, label }) => (
              <Button key={itemView} variant={view === itemView ? 'default' : 'ghost'} onClick={() => setView(itemView as View)}>
                ...
                {label}
                {itemView === 'messages' && unread > 0 && <Badge className="ml-2">{unread}</Badge>}
              </Button>
            ))}
          </div>
        </Card>
      ...
      <main className="mx-auto w-full max-w-7xl space-y-6 py-6 pb-20 md:pb-8">
        {view === 'home' && (
            ...
            <div className="space-y-6 px-4 sm:px-6">
              <section className="rounded-2xl bg-gradient-to-br from-[#002F6C] to-[#0055A5] p-6 text-white shadow-large sm:p-8">
                ...
                <div className="mt-2 flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
                  <div>
                    <h2 className="text-2xl font-extrabold sm:text-3xl">Your Pilot safety dashboard is live</h2>
                    <p className="mt-2 text-white/80">Reports, evidence, location, status updates and notifications run end to end inside the isolated Pilot environment.</p>
                  </div>
                  <Button className="bg-[#F2A900] font-bold text-[#002F6C]" onClick={() => setView('report')}>
                    <Plus className="mr-2 h-4 w-4" />Report an Incident
                  </Button>
                </div>
              </section>
            ...
            <Card>
              <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Submitted records appear immediately in staff Pilot dashboards.</p>
                </div>
        ...
        {view === 'map' && (
          <div className="px-4 sm:px-6">
            <Card>
              <CardHeader>
                <CardTitle>Location and Tracking</CardTitle>
                <CardDescription>Select a location-enabled workflow to test GPS and live tracking.</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-4 md:grid-cols-2">
                {scenarios.filter((item) => item.requires_location || item.requires_live_tracking).map((item) => (
                  <button
                    key={item.id}
                    className="rounded-xl border p-5 text-left hover:border-primary"
                    onClick={() => { setScenarioId(item.id); setView('report'); }}
                  >
          ...
          <div className="px-4 sm:px-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Live updates from authorised Pilot staff.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {notifications.map((item) => (
                  <button
                    key={item.id}
                    className={`w-full rounded-xl border p-4 text-left ${item.is_read ? '' : 'border-[#F2A900] bg-[#F2A900]/10'}`}
                    onClick={async () => {
                      if (!item.is_read) {
                        await markPilotNotificationRead(item.id);
                        await refresh();
                      }
                    }}
                  >
                    <div className="flex justify-between gap-3">
                      ...
                      {!item.is_read && <Bell className="h-5 w-5 text-[#F2A900]" />}
                    </div>
                  </button>
                ))}
              </CardContent>
  ...
  return (
    <Card className="shadow-large">
      <CardHeader>
        ...
        <CardDescription>Live Pilot records remain separate from production cases.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {reports.map((report) => (
          <Link key={report.id} to={PILOT_ROUTES.report(report.id)} className="flex items-center justify-between rounded-xl border p-4 hover:border-primary">
            <div>
              <p className="font-bold">{report.title}</p>
              <p className="text-sm text-muted-foreground">{report.reference_number} · {PILOT_STATUS_LABELS[report.status]}</p>
            </div>
            <ChevronRight className="h-5 w-5" />
          </Link>
        ))}
        {!reports.length && <p className="py-8 text-center text-muted-foreground">No Pilot reports yet.</p>}
      </CardContent>
    </Card>
  );
}
