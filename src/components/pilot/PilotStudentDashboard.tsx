...
import {
  loadPilotCarouselSlides,
  loadPilotSafetyDocument,
  PILOT_SAFETY_GUIDE_FALLBACK,
  subscribeToPilotCarouselSlides,
...
export function PilotStudentDashboard({
  ...
  const [slides, setSlides] = useState<PilotCarouselSlide[]>([]);
  const [safetyDocument, setSafetyDocument] = useState<PilotSafetyDocument>(PILOT_SAFETY_GUIDE_FALLBACK);
  ...
  const refreshExperience = useCallback(async () => {
    ...
    try {
      const [nextSlides, nextDocument] = await Promise.all([
        loadPilotCarouselSlides(program.id),
        loadPilotSafetyDocument(program.id),
      ]);
      setSlides(nextSlides);
      setSafetyDocument(nextDocument);
    } catch (error) {
      setSlides([]);
      setSafetyDocument(PILOT_SAFETY_GUIDE_FALLBACK);
      toast({
        title: 'Pilot information content unavailable',
        description: error instanceof Error ? error.message : 'Reporting remains available.',
        variant: 'destructive',
      });
    } finally {
  ...
  useEffect(() => { void refreshExperience(); }, [refreshExperience]);
  useEffect(() => subscribeToPilotNotifications(participant.user_id, () => void refresh()), [participant.user_id, refresh]);
  useEffect(
    () => subscribeToPilotCarouselSlides(program.id, participant.campus, () => void refreshExperience()),
    [participant.campus, program.id, refreshExperience],
  );
  ...
  const openEmergencySimulation = () => {
    if (!emergencyScenario) {
      toast({
        title: 'Emergency simulation unavailable',
        description: 'No authorised emergency scenario is active for this Pilot programme.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleCarouselAction = (action: PilotCarouselAction) => {
    if (action === 'report') setView('report');
    else if (action === 'emergency') openEmergencySimulation();
    else if (action === 'cases') setView('mycases');
    else if (action === 'reviews') navigate(PILOT_ROUTES.reviews);
    else if (action === 'resources') navigate(PILOT_ROUTES.resources);
  ...
  const markRead = async (notification: PilotNotification) => {
    ...
    try {
      ...
      toast({ title: 'Notification was not updated', description: error instanceof Error ? error.message : 'Try again.', variant: 'destructive' });
    }
  };

  const downloadSafetyGuide = async () => {
    await recordPilotFeatureTest({
      programId: program.id,
      sessionId: session.id,
      featureKey: 'safety_guide_pdf_download',
      outcome: 'passed',
      metadata: { version: safetyDocument.version, url: safetyDocument.download_url },
    }).catch(() => undefined);

    const anchor = document.createElement('a');
    anchor.href = safetyDocument.download_url;
    anchor.download = `CCSF-Pilot-Safety-Guide-v${safetyDocument.version}.pdf`;
  ...
  if (loading) {
    return <div className="flex min-h-[55vh] items-center justify-center" role="status" aria-label="Loading student Pilot dashboard"><Loader2 className="h-9 w-9 animate-spin text-primary" /></div>;
  }
      ...
      <div className="mx-auto max-w-xl px-4 py-12">
        <Card className="border-destructive/30 text-center shadow-large">
          <CardContent className="space-y-4 p-8">
            ...
            <Button onClick={() => void refresh(true)} disabled={refreshing}>{refreshing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}Retry dashboard</Button>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <main className="mx-auto w-full max-w-7xl space-y-6 py-6 pb-24 md:pb-8">
              ...
              <section className="rounded-2xl bg-gradient-to-br from-[#002F6C] to-[#0055A5] p-6 text-white shadow-large sm:p-8">
                ...
                <div className="mt-2 flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    ...
                    <Button variant="destructive" className="font-bold" onClick={openEmergencySimulation}><Siren className="mr-2 h-4 w-4" />Emergency Test</Button>
                  </div>
                </div>
              </section>
        ...
        {view === 'report' && (
          <div className="space-y-5 px-4 sm:px-6">
            ...
            {selectedScenario ? (
              <PilotReportForm key={selectedScenario.id} scenario={selectedScenario} participant={participant} session={session} emergency={selectedScenario.scenario_type === 'emergency_simulation'} />
            ) : (
        ...
        {view === 'map' && (
          <div className="space-y-5 px-4 sm:px-6">
            ...
            <Card>
              <CardHeader><CardTitle>Cases with captured locations</CardTitle><CardDescription>{locationReports.length} Pilot case(s) currently include a location fix.</CardDescription></CardHeader>
              <CardContent className="space-y-3">
                {locationReports.map((report) => (
                  <Link key={report.id} to={PILOT_ROUTES.report(report.id)} className="flex items-center justify-between gap-4 rounded-xl border p-4 transition hover:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                    <div className="min-w-0"><p className="font-bold">{report.title}</p><p className="mt-1 text-sm">{report.location_description || 'Readable address unavailable'}</p>{report.location_lat !== null && report.location_lng !== null && <p className="mt-1 text-xs text-muted-foreground">{formatCoordinatePair(report.location_lat, report.location_lng)}</p>}</div>
          ...
          <div className="grid gap-5 px-4 sm:px-6 lg:grid-cols-[0.8fr_1.2fr]">
            ...
            <Card>
              <CardHeader className="flex-row items-start justify-between gap-3">
                <div><CardTitle>Pilot Notifications</CardTitle><CardDescription>Realtime updates from authorised Pilot staff.</CardDescription></div>
                <Button variant="outline" size="icon" onClick={() => void refresh(true)} disabled={refreshing} aria-label="Refresh Pilot notifications">{refreshing ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}</Button>
              </CardHeader>
              <CardContent className="space-y-3" role="log" aria-live="polite">
                {notifications.map((item) => (
                  <button key={item.id} className={`w-full rounded-xl border p-4 text-left ${item.is_read ? '' : 'border-[#F2A900] bg-[#F2A900]/10'}`} onClick={() => void markRead(item)}>
...
function QuickAction({
  ...
  return <button onClick={onClick} className="rounded-xl border bg-card p-5 text-left shadow-sm transition hover:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Icon className="h-5 w-5 text-primary" /><p className="mt-3 font-bold">{title}</p><p className="mt-1 text-sm text-muted-foreground">{description}</p></button>;
}
...
function CaseList({ reports, title }: { reports: PilotReport[]; title: string }) {
  ...
  return (
    <Card className="shadow-large">
      <CardHeader><CardTitle>{title}</CardTitle><CardDescription>Tap a case to open its full details, readable location, evidence and timeline.</CardDescription></CardHeader>
      <CardContent className="space-y-3">
        {sorted.map((report) => (
          <Link key={report.id} to={PILOT_ROUTES.report(report.id)} className="flex items-center justify-between gap-4 rounded-xl border p-4 transition hover:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            <div className="min-w-0"><p className="truncate font-bold">{report.title}</p><p className="text-sm text-muted-foreground">{report.reference_number} · {PILOT_STATUS_LABELS[report.status]}</p>{report.location_description && <p className="mt-1 truncate text-sm"><MapPin className="mr-1 inline h-3.5 w-3.5 text-primary" />{report.location_description}</p>}</div>
            <ChevronRight className="h-5 w-5 shrink-0" />
          </Link>
        ))}
        {!sorted.length && <p className="py-8 text-center text-muted-foreground">No Pilot reports yet.</p>}
      </CardContent>
    </Card>
  );
}