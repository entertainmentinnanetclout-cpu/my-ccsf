...
export default function SplashScreen({ onComplete, minDuration = 1200 }: SplashScreenProps) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[9999] flex min-h-screen items-center justify-center overflow-hidden border-t-4 border-t-[#F2A900] bg-[#002F6C] px-5 text-white"
      initial={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      ...
      <div className="relative w-full max-w-2xl text-center">
        <motion.div initial={reduceMotion ? false : { opacity: 0, scale: 0.94, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.45, ease: 'easeOut' }}>
        ...
        <button type="button" onClick={onComplete} className="mt-7 text-xs font-semibold text-white/55 underline-offset-4 hover:text-white hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F2A900]">
          Continue now
        </button>
      </div>
    </motion.div>
