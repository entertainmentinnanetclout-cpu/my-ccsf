...
export function MobileEvidencePicker({
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Clearing the native value allows a student to choose the same file again after validation.
      ...
      {files.length > 0 && (
        <div className="rounded-lg border border-emerald-500/25 bg-emerald-500/10 p-3 text-sm">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold">{files.length} file{files.length === 1 ? '' : 's'} attached</p>
              <p className="mt-1 break-words text-muted-foreground">{files.map((file) => file.name).join(', ')}</p>
            </div>
          </div>
        </div>
      )}
