...
const CAMPUSES = [{
  value: 'pretoria_west_main',
  label: 'Pretoria West (Main)'
}, {
  value: 'arcadia',
  label: 'Arcadia Campus'
}, {
  value: 'arts',
  label: 'Arts Campus'
}, {
  ...
  value: 'mbombela',
  label: 'Mbombela Campus'
}, {
  value: 'polokwane',
  label: 'Polokwane Campus'
}, {
  value: 'garankuwa',
  label: 'Ga-Rankuwa Campus'
}, {
  value: 'soshanguve_south',
  label: 'Soshanguve South'
}, {
  value: 'soshanguve_north',
  label: 'Soshanguve North'
}, {
  value: 'emalahleni',
  label: 'eMalahleni Campus'
}];
...
export const CampusAdminManager = () => {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      const {
        data: accessData,
        error: accessError
      } = await supabase.from('admin_access').select('id, admin_id, campus, is_head').order('campus', {
        ascending: true
      });
      if (accessError) {
  ...
  const handleAssign = async () => {
    if (!formData.userId || !formData.campus) {
      toast({
        title: 'Please select a user and campus',
        variant: 'destructive'
      });
      return;
    }
    const {
      error
    } = await supabase.rpc('assign_campus_admin', {
    ...
    if (error) {
      ...
      toast({
        title: 'Campus officer assigned successfully'
      });
      fetchAdmins();
  ...
  const handleCreateAdmin = async () => {
    ...
    try {
      ...
      toast({
        title: 'CCSF Officer created successfully',
        description: `${createFormData.fullName} can now log in with their credentials`
      });
      fetchAdmins();
      setIsDialogOpen(false);
      setCreateFormData({
  ...
  const handleUpdateAdmin = async () => {
    ...
    try {
      ...
      if (editFormData.campus !== editingAdmin.campus) {
        ...
        const {
          error
        } = await supabase.from('admin_access').update({
          is_head: editFormData.isHead
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-primary-foreground font-medium">Create and manage Campus Community Safety Forum officers</p>
        </div>
      ...
      {isLoading ? <div className="flex items-center justify-center h-64">
          ...
          {Object.entries(adminsByCampus).map(([campus, campusAdmins]) => <motion.div key={campus} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {campusAdmins.map(admin => <div key={admin.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {admin.is_head && <Badge variant="default" className="text-xs">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => handleRemove(admin.admin_id, admin.campus)} className="text-destructive hover:text-destructive">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={editFormData.campus} onValueChange={v => setEditFormData({
              ...editFormData,
              campus: v
            })}>
                <SelectTrigger>
                ...
                <SelectContent>
                  {CAMPUSES.map(campus => <SelectItem key={campus.value} value={campus.value}>{campus.label}</SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="editIsHead" checked={editFormData.isHead} onChange={e => setEditFormData({
              ...editFormData,
          ...
          <Tabs value={dialogTab} onValueChange={v => setDialogTab(v as 'assign' | 'create')}>
            ...
            <TabsContent value="create" className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input id="fullName" placeholder="Officer's full name" value={createFormData.fullName} onChange={e => setCreateFormData({
                ...createFormData,
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="officer@email.com" value={createFormData.email} onChange={e => setCreateFormData({
                ...createFormData,
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Minimum 6 characters" value={createFormData.password} onChange={e => setCreateFormData({
                  ...createFormData,
              ...
              <div className="space-y-2">
                <Label>Campus *</Label>
                <Select value={createFormData.campus} onValueChange={v => setCreateFormData({
                ...createFormData,
                campus: v
              })}>
                  <SelectTrigger>
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="createIsHead" checked={createFormData.isHead} onChange={e => setCreateFormData({
                ...createFormData,
                isHead: e.target.checked
              })} className="rounded" />
            ...
            <TabsContent value="assign" className="space-y-4 pt-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.userId} onValueChange={v => setFormData({
                ...formData,
                userId: v
              })}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {filteredUsers.slice(0, 20).map(user => <SelectItem key={user.id} value={user.id}>
              ...
              <div className="space-y-2">
                <Label>Campus</Label>
                <Select value={formData.campus} onValueChange={v => setFormData({
                ...formData,
                campus: v
              })}>
                  <SelectTrigger>
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="isHead" checked={formData.isHead} onChange={e => setFormData({
                ...formData,
                isHead: e.target.checked
              })} className="rounded" />
