...
const MIME_BY_EXTENSION: Record<string, string> = {
  jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', webp: 'image/webp', heic: 'image/heic', heif: 'image/heif', mp4: 'video/mp4', pdf: 'application/pdf',
};
...
export async function prepareEvidenceFile(file: File, options: EvidencePreparationOptions): Promise<File> {
  ...
  if (!isAllowedEvidenceFile(file, options.allowPdf)) throw new Error(`${file.name} is not an accepted evidence format.`);
  const prepared = IMAGE_TYPES.has(normaliseEvidenceMimeType(file)) ? await convertOrCompressImage(file, {
