...
export function evidenceFileIdentity(file: File): string {
  return `${file.name}:${file.size}:${file.lastModified}:${normaliseEvidenceMimeType(file)}`;
}

export function isAllowedEvidenceFile(file: File, allowPdf = false): boolean {
  const type = normaliseEvidenceMimeType(file);
  return IMAGE_TYPES.has(type) || type === 'video/mp4' || (allowPdf && type === 'application/pdf');
}
...
async function loadImageSource(file: File): Promise<{ source: CanvasImageSource; width: number; height: number; close: () => void }> {
  if ('createImageBitmap' in window) {
    try {
      ...
      // Safari may decode HEIC through an HTML image even when createImageBitmap cannot.
    }
  }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error(`${file.name} could not be decoded on this device.`));
    image.src = url;
  });
...
function canvasBlob(canvas: HTMLCanvasElement, type: string, quality: number): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('The image could not be converted.')), type, quality);
  });
}
...
async function convertOrCompressImage(file: File, options: Required<Pick<EvidencePreparationOptions, 'maxDimension' | 'compressAboveBytes' | 'jpegQuality'>>): Promise<File> {
  ...
  if (!needsConversion && !needsCompression) return file;
  ...
  try {
    ...
    const converted = new File([blob], `${baseName}.${outputExtension}`, {
      type: outputType,
      lastModified: file.lastModified || Date.now(),
    });
    return converted.size < file.size || needsConversion ? converted : file;
  } finally {
    loaded.close();
  }
}
...
export async function prepareEvidenceFile(file: File, options: EvidencePreparationOptions): Promise<File> {
  ...
  if (!isAllowedEvidenceFile(file, options.allowPdf)) {
    throw new Error(`${file.name} is not an accepted evidence format.`);
  }

  const prepared = IMAGE_TYPES.has(normaliseEvidenceMimeType(file))
    ? await convertOrCompressImage(file, {
  ...
  if (prepared.size > options.maxBytes) {
    throw new Error(`${prepared.name} is ${(prepared.size / (1024 * 1024)).toFixed(1)} MB after processing and exceeds the ${(options.maxBytes / (1024 * 1024)).toFixed(0)} MB limit.`);
  }
  return prepared;
}
