...
function resumeKey(bucket: string, path: string, file: File): string {
  return `${RESUME_PREFIX}:${bucket}:${path}:${evidenceFileIdentity(file)}`;
}
...
function readResume(key: string): ResumeRecord | null {
  try {
    ...
    if (!record.url || Date.now() - record.createdAt > MAX_RESUME_AGE_MS) {
...
function writeResume(key: string, record: ResumeRecord): void {
  try { localStorage.setItem(key, JSON.stringify(record)); } catch { /* Resumability is best-effort. */ }
}

function clearResume(key: string): void {
  try { localStorage.removeItem(key); } catch { /* Ignore storage cleanup failures. */ }
}
...
async function currentOffset(url: string, token: string, signal?: AbortSignal): Promise<number | null> {
  const response = await fetch(url, {
    method: 'HEAD',
    headers: { authorization: `Bearer ${token}`, 'Tus-Resumable': TUS_VERSION },
    signal,
  });
...
export async function uploadResumableEvidence(options: ResumableUploadOptions): Promise<{ path: string; resumed: boolean }> {
  ...
  if (!record) {
    record = { url, createdAt: Date.now(), fileIdentity: evidenceFileIdentity(options.file), bucket: options.bucket, path: options.path };
    writeResume(key, record);
  }

  let offset: number | null;
  try {
    offset = await currentOffset(url, token, options.signal);
  } catch {
    token = await accessToken();
    offset = await currentOffset(url, token, options.signal).catch(() => null);
  }

  if (offset === null) {
    clearResume(key);
    resumed = false;
    url = await createUpload(options, token);
