...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: plan, error: planError } = await context.callerClient.rpc('pilot_delete_report', {
    ...
    const { data: result, error: finaliseError } = await context.adminClient.rpc('pilot_finalize_delete_report', {
      p_report_id: reportId,
      p_reason: reason,
