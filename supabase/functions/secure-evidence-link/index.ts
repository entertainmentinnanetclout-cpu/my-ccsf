...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let recordCampus: string | null = null;
    ...
    if (scope === 'official') {
      ...
      authorised = incident.submitted_by === user.id
        || incident.reporter_id === user.id
        || incident.assigned_to === user.id
        || role === 'admin'
        || (role === 'security' && Boolean(campus) && campus === incident.campus);
      ...
      authorised = report.submitted_by === user.id
        || role === 'admin'
        || (role === 'security' && Boolean(campus) && campus === report.campus);
    ...
    const { data: signed, error: signedError } = await admin.storage.from(bucket).createSignedUrl(objectPath, expiresIn, {
      download: action === 'download',
    });
    if (signedError || !signed?.signedUrl) throw signedError ?? new Error('A private evidence link could not be created.');

    const { error: auditError } = await admin.from('evidence_access_audit').insert({
      scope,
      bucket_id: bucket,
      ...
      action,
      reason,
      metadata: { expires_in_seconds: expiresIn, record_campus: recordCampus, user_agent: req.headers.get('user-agent') },
    });
