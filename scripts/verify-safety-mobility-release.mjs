...
check(hook.includes('browser tracking works') === false, 'Hook contains no misleading guarantee of permanent browser tracking.');
...
check(hub.includes('Approximate users are deliberately blurred') && hub.includes('tappable profile'), 'Radar UI explains its interactive, privacy-limited behavior.');
...
check(icon.includes('fill="#ffffff"') && icon.includes('Campus safety') === false && icon.includes('MY CCSF'), 'Generated app icon has a permanent white background and visible MY CCSF label.');
