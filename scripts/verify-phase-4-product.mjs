...
for (const route of ['/dashboard', '/security/*', '/admin/*', '/office', '/profile', '/profile-completion', '/judiciary']) check(app.includes(`path="${route}"`), `Primary route ${route} remains registered.`);
for (const label of ['Home', 'My Cases', 'Report', 'Safety', 'Support']) check(dashboard.includes(`label: '${label}'`), `Student dashboard includes ${label}.`);
...
for (const marker of ['In-Transit', 'Night Travel', 'Track This Phone', 'Campus Radar', '<InstitutionalCampusRadar', 'selectedStudent', 'loadCampusRadar']) check(hub.includes(marker), `Safety Mobility UI includes ${marker}.`);
